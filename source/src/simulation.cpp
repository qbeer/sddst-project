...
double get_distance(Dislocation d1, Dislocation d2) {
    double sum = 0.0;
    sum += (d1.x - d2.x)*(d1.x - d2.x);
    sum += (d1.y - d2.y)*(d1.y - d2.y);
    return sqrt(sum);
}
...
void Simulation::stepStageIII()
{
    ...
    if (pH->getMaxErrorRatioSqr() < 1.0)
    {
        ...
        std::vector<double> min_distances(sD->dislocations.size());

        for(unsigned int i = 0; i < sD->dislocations.size(); i++){
            double min_dist = HUGE_DISTANCE;
            
            for(unsigned int j = 0; j < i; j++){
                double current_dist = get_distance(sD->dislocations[i], sD->dislocations[j]);
                if( current_dist < min_dist) {
                    min_dist = current_dist;
                }
            }

            for(unsigned int j = i + 1; j < sD->dislocations.size(); j++){
                double current_dist = get_distance(sD->dislocations[i], sD->dislocations[j]);
                if( current_dist < min_dist) {
                    min_dist = current_dist;
                }
        ...
        std::cout << sum/min_distances.size() << " ";
